...
**Company lens (independent):** Supabase (`supabase`)  
**Innovation:** Policy-Carrying AI Data Plane  
**Scaffold batch:** 2026-08-10T0924Z

## Mission

Implement a **real, testable** central mechanism that addresses the bottleneck below. Do **not** claim Supabase affiliation, proprietary access, or production deployment.
...
Propagate row-level security identity and policy into embedding generation, retrieval, tool execution and generated artifacts so AI never leaves the authorization model of the source data.

## Hard rules (fail closed)

1. **No affiliation theater** — never state or imply Supabase employment, endorsement, or proprietary systems access.
...
File: `src/policy_carrying_ai_data_plane.py`

- Expand `PolicyCarryingAiDataPlane` into a complete, self-contained implementation.
- Public API must stay stable enough that tests in `tests/test_policy_carrying_ai_data_plane.py` can be upgraded (not gutted).
- Include at least:
...
### 3. Strengthen tests
Files: `tests/test_policy_carrying_ai_data_plane.py`, `tests/test_adversarial.py`
...
- Cloud deploy, customer pilots, proprietary Supabase APIs
